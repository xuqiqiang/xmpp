...
public class PopupPromptView {
    ...
    private void initPopupWindowLayout() {
        ...
        this.popup_window_layout = (LinearLayout) mInflater.inflate(
                R.layout.popup_menu_prompt_layout, null);
        popup_window_content = (ViewGroup) popup_window_layout
                .findViewById(R.id.popup_window_content);
                ...
                public boolean onPreDraw() {
                    ...
                    if (onTop) {
                        popup_window_content
                                .setBackgroundResource(R.drawable.tip_bottom_bg);
                        yoff = -view.getHeight()
                                - popup_window_layout.getMeasuredHeight()
                                + (int) DisplayUtils.dip2px(context, 4);
                    } else {
                        popup_window_content
                                .setBackgroundResource(R.drawable.tip_top_bg);
                        yoff = -(int) DisplayUtils.dip2px(context, 4);
                    }
                    dismiss();
                    try {
                        // this.mPopupWindow.showAtLocation(view, 51,
    ...
    public static void show(Context context, int resId, final View view,
            final boolean onTop) {
        show(context, context.getString(resId), view, onTop);
    }

    public static void show(Context context, String message, final View view,
            final boolean onTop) {
    ...
    public void dismissDelay() {
        ObjectAnimator hiddenNewButtonAnimator = ObjectAnimator.ofFloat(
                popup_window_layout, "alpha", 1, 0);
        AnimatorSet mAnimation = new AnimatorSet().setDuration(HIDDEN_DURATION);
        mAnimation.play(hiddenNewButtonAnimator);
        mAnimation.start();
